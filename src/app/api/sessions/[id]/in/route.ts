...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const open = await prisma.parkingSession.findFirst({ where: { userId: user.id, outAt: null } });
  if (open) {
    return NextResponse.json({
      ok: false,
      reason: "already_in",
      message: "IN is already recorded. You can’t log another IN before you OUT.",
    }, { status: 409 });
  }

  await prisma.parkingSession.create({ data: { userId: user.id, inAt: new Date(), sourceLat: lat, sourceLng: lng } });
